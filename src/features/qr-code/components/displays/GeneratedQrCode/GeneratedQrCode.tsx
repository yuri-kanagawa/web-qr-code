...
import { Box, QRCode, Stack } from '@/ui/cores'
...
const GeneratedQrCode = React.forwardRef<HTMLDivElement, Props>(
  (
    ...
    const maxSize = useMemo(() => {
      ...
      // ただし、小さいサイズの場合は50を引かない（負の値にならないように）
      if (propHeight && propWidth) {
        const minSize = Math.min(propHeight, propWidth)
        return Math.max(50, minSize) // 最小50pxを保証
      }
    ...
    }, [propHeight, propWidth, isOverLaptop])
    ...
    return (
      <>
        {!hidden && (
          <Stack p={isOverLaptop ? 8 : 4}>
            <Box sx={{ display: 'inline-block', m: isOverLaptop ? 2 : 0 }}>
              <CornerHighlightBox height={maxSize + 50} width={maxSize + 50}>
                <div
                  ref={ref}
                  style={{
                    position: 'relative',
                    padding: isOverLaptop ? '16px' : '0'
                  }}
                >
                  {isValid &&
                    qr.qrValue.value &&
                    qr.qrValue.value.trim() !== '' && (
                      <>
                        {(() => {
                          const logoProps =
                            logoImage &&
                            safeQr.settings.logo.width &&
                            safeQr.settings.logo.height
                              ? {
                                  logoImage,
